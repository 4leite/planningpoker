...
import { RoomCenter } from "#/components/planning-poker/room/RoomCenter";
  ...
  return (
    <div className="pointer-events-none absolute inset-0">
      {members.map((member, index) => {
        const angle =
          ((Math.PI * 2) / Math.max(members.length, 1)) * index - Math.PI / 2;
        ...
        const isLowestOutlier =
          member.vote !== null && member.vote === lowestVote;
        const isHighestOutlier =
          member.vote !== null && member.vote === highestVote;
        ...
        return (
          ...
          >
            <div
              className={cn(
                ...
                isUnresolvedParticipant &&
                  "bg-amber-500/10 ring-1 ring-amber-400/60 ring-inset",
                isModeVote &&
                  "bg-emerald-500/10 ring-1 ring-emerald-400/60 ring-inset",
                isLowestOutlier &&
                  "bg-sky-500/10 ring-1 ring-sky-400/60 ring-inset",
                isHighestOutlier &&
                  "bg-sky-500/10 ring-1 ring-sky-400/60 ring-inset",
            ...
            >
              ...
              <div className="truncate pr-5 text-sm font-medium">
                {member.name}
              </div>
              <div
                className={cn(
